...
const NavBarLayout = () => {
  ...
  const exportResumeData = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/saveResume", {
        ...
        body: JSON.stringify({ updatedResumeJson }),
      });
  ...
  return (
    <nav className="h-14 w-full bg-gray-900 relative flex py-2.5 pl-2 md:pl-5 pr-1 nd:pr-4 items-center shadow-level-8dp z-20 print:hidden">
      <Link href="/">
      ...
      <div className="flex-auto flex justify-between items-center xs:ml-3 md:ml-5">
        ...
        <button className="flex md:hidden text-white" onClick={handleMenuOpen} aria-label="Open menu">
      ...
      >
        <MenuItem onClick={exportResumeData}>I am Done</MenuItem>
        {/* <MenuItem
          onClick={() => {
            if (fileInputRef.current) {
              const fileElement = fileInputRef.current as HTMLInputElement;
